...
parser.add_argument('-m', '--trained_model', default='./weights/mobilenet0.25_quantized.pth', type=str, help='Trained state_dict file path to open')
...
class ros_fddb:
    def __init__(self):
        ...
        self.cfg = cfg_mnet if args.network == "mobile0.25" else cfg_re50
        self.device = torch.device("cpu" if args.cpu else "cuda")

        # 양자화된 모델 로드
        self.net = self.load_model(args.trained_model)
    ...
    def load_model(self, pretrained_path):
        ...
        # 불필요한 키 제거
        filtered_state_dict = {k: v for k, v in state_dict.items() if 'scale' not in k and 'zero_point' not in k}
        for k, v in filtered_state_dict.items():
            if v.is_quantized:
                filtered_state_dict[k] = v.dequantize()
